...
private class CxPingProgress : public CkBaseProgressW
    {
	

    public:
	Chilkat::Ping ^m_sender;
	CkPingW *m_impl;
	
    private:
	 	CxPingProgress(void) : m_impl(0)
		    {
		    m_sender = nullptr;
		    }
   	
	public:
	CxPingProgress(CkPingW *pImpl) : m_impl(pImpl)
	    {
	    m_sender = nullptr;
	    m_impl->put_EventCallbackObject(this);
	    }

	virtual ~CxPingProgress(void)
	    {
	    m_sender = nullptr;
