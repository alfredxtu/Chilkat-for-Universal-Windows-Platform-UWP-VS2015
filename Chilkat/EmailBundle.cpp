...
Boolean EmailBundle::AddEmail(Chilkat::Email ^email)
    {
...
Email ^EmailBundle::FindByHeader(Platform::String ^headerFieldName, Platform::String ^headerFieldValue)
    {
	if (m_impl == nullptr) { return nullptr; }
	// --- prep output arg ---
	CkEmailW *pRetObj = m_impl->FindByHeader(headerFieldName ? headerFieldName->Data() : L"",headerFieldValue ? headerFieldValue->Data() : L"");
...
Boolean EmailBundle::RemoveEmail(Chilkat::Email ^email)
    {
	if (m_impl == nullptr) { return false; }
	if (email == nullptr) { return false; }
	CkEmailW* pObj0 = email->m_impl;
